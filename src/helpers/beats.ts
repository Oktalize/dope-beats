const dopeSounds = {
  vehicle: {
      ...
      "golf cart": "https://ipfs.infura.io/ipfs/QmQV4xJA5y7WRoernbtgXhKsf3ZzEiBQ7t2Fiy5YdZEuZ1",
      ...
      "bike": "https://ipfs.infura.io/ipfs/QmdSu1Bj5JNnDudELmbdXUTryEFKCg786oBCcuEP6i6kL3",
...
export function soundFromBagItems(items: Item[]): string[] {
  return items.reduce((res, i) => {
    const { slot, name } = i;
    const [prefix] = name.split(" from ");
    const [root] = prefix.split('\"');
    if (dopeSounds.hasOwnProperty(slot)) {
      const audio: string = (dopeSounds as any)[slot][root.toLowerCase()];
      if (!audio) {
        console.log(slot, name);
      }
      if (!!audio) res.push(audio);
    }
    return res;
  }, [] as string[]);
}